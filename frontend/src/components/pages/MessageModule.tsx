...
// IMPROVED: Added specific types for better code safety
interface Message {
    sender: 'me' | 'other';
    text: string;
    time: string;
...
const MessagesModule: React.FC<MessagesModuleProps> = ({ onNavigate, userName }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-civix-sandal to-civix-warm-beige dark:from-gray-900 dark:to-gray-800 p-6">
        <div className="flex items-center justify-between mb-6">
            <div>
                <h2 className="text-3xl font-bold text-civix-dark-brown dark:text-white">Messages</h2>
                <p className="text-gray-500 dark:text-gray-400">Conversations with your community leaders, {userName}.</p>
            </div>
                            ...
                            {selectedConversation.messages.map((msg, index) => (
                                <div key={index} className={`flex items-end gap-3 ${msg.sender === 'me' ? 'justify-end' : ''}`}>
                                    ...
                                    <div className={`max-w-xs md:max-w-md p-3 rounded-2xl ${msg.sender === 'me' ? 'bg-civix-civic-green text-white rounded-br-none' : 'bg-white dark:bg-gray-700 text-civix-dark-brown dark:text-gray-200 rounded-bl-none'}`}>
                                        ...
                                        <p className={`text-xs mt-1 text-right ${msg.sender === 'me' ? 'text-white/70' : 'text-gray-400'}`}>{msg.time}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
